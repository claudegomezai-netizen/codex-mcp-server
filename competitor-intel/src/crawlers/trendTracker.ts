...
export async function captureSnapshot(): Promise<number> {
  ...
  try {
    ...
    for (const entity of allEntities) {
      try {
        ...
        const last24h = articles.filter(a => {
          ...
          return !isNaN(t) && t > Date.now() - 24 * 3600000;
        });
        if (last24h.length > 0) {
        ...
        console.warn(`[Trends] Error processing articles for ${entity.name}:`, err);
      }
    }
    ...
    added = await addTrendSnapshots(snapshots);
    ...
    try {
      ...
      console.warn('[Trends] logCrawl failed:', logErr);
    }
  } catch (err: any) {
    console.error(`[Trends] Snapshot failed: ${err.message}`);
    try {
      await logCrawl({
        crawl_type: 'trend_snapshot',
        entity_id: null,
