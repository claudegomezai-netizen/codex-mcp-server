...
function summaryStore() {
  return getStore({ name: 'filing-summaries', consistency: 'strong' });
}

export async function getFilingSummaries(): Promise<FilingSummary[]> {
  const store = summaryStore();
  return ((await store.get('all', { type: 'json' })) as FilingSummary[]) || [];
}

export async function getFilingSummary(filingId: string): Promise<FilingSummary | null> {
  const all = await getFilingSummaries();
  return all.find(s => s.filing_id === filingId) || null;
}

export async function saveFilingSummary(summary: FilingSummary): Promise<void> {
  const store = summaryStore();
  const existing = await getFilingSummaries();
  const idx = existing.findIndex(s => s.filing_id === summary.filing_id);
  if (idx >= 0) existing[idx] = summary;
  else existing.unshift(summary);
...
function advStore() {
  return getStore({ name: 'adv-analysis', consistency: 'strong' });
}

export async function getAdvAnalyses(): Promise<FormAdvAnalysis[]> {
  const store = advStore();
  return ((await store.get('all', { type: 'json' })) as FormAdvAnalysis[]) || [];
}

export async function saveAdvAnalysis(analysis: FormAdvAnalysis): Promise<void> {
  const store = advStore();
  const existing = await getAdvAnalyses();
  const idx = existing.findIndex(a => a.entity_id === analysis.entity_id);
  if (idx >= 0) existing[idx] = analysis;
  else existing.push(analysis);
  await store.setJSON('all', existing);
}
...
function trendStore() {
  return getStore({ name: 'trend-history', consistency: 'strong' });
}
...
export async function getTrendSnapshots(filters?: {
  ...
  let all: TrendSnapshot[] = ((await store.get('all', { type: 'json' })) as TrendSnapshot[]) || [];
  if (filters?.metricType) all = all.filter(t => t.metric_type === filters.metricType);
  if (filters?.entityId) all = all.filter(t => t.entity_id === filters.entityId);
  if (filters?.startDate) all = all.filter(t => t.date >= filters.startDate!);
  if (filters?.endDate) all = all.filter(t => t.date <= filters.endDate!);
  return all.sort((a, b) => a.date.localeCompare(b.date));
}

export async function addTrendSnapshots(snapshots: TrendSnapshot[]): Promise<number> {
  const store = trendStore();
  const existing: TrendSnapshot[] = ((await store.get('all', { type: 'json' })) as TrendSnapshot[]) || [];
  const existingKeys = new Set(existing.map(t => `${t.date}:${t.metric_type}:${t.entity_id}`));
  const unique = snapshots.filter(t => !existingKeys.has(`${t.date}:${t.metric_type}:${t.entity_id}`));
  if (unique.length === 0) return 0;
  const merged = [...existing, ...unique].sort((a, b) => a.date.localeCompare(b.date)).slice(-5000);
  await store.setJSON('all', merged);
  return unique.length;
}
...
function personnelStore() {
  return getStore({ name: 'personnel-changes', consistency: 'strong' });
}

export async function getPersonnelChanges(filters?: {
  entityId?: string;
  changeType?: string;
}): Promise<PersonnelChange[]> {
  const store = personnelStore();
  let all: PersonnelChange[] = ((await store.get('all', { type: 'json' })) as PersonnelChange[]) || [];
  if (filters?.entityId) all = all.filter(p => p.entity_id === filters.entityId);
  if (filters?.changeType) all = all.filter(p => p.change_type === filters.changeType);
  return all.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
}

export async function addPersonnelChanges(changes: PersonnelChange[]): Promise<number> {
  const store = personnelStore();
  const existing = await getPersonnelChanges();
  const existingIds = new Set(existing.map(p => p.id));
  const unique = changes.filter(p => !existingIds.has(p.id));
  if (unique.length === 0) return 0;
  const merged = [...unique, ...existing]
    .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
    .slice(0, 500);
...
// ── Mandate Events (RFP / Win-Loss) ─────────────────────

function mandateStore() {
  return getStore({ name: 'mandate-events', consistency: 'strong' });
}

export async function getMandateEvents(filters?: {
  entityId?: string;
  eventType?: string;
}): Promise<MandateEvent[]> {
  const store = mandateStore();
  let all: MandateEvent[] = ((await store.get('all', { type: 'json' })) as MandateEvent[]) || [];
  if (filters?.entityId) all = all.filter(m => m.entity_id === filters.entityId);
  if (filters?.eventType) all = all.filter(m => m.event_type === filters.eventType);
  return all.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
}

export async function addMandateEvent(event: MandateEvent): Promise<void> {
  const store = mandateStore();
  const existing = await getMandateEvents();
  if (existing.some(m => m.id === event.id)) return;
  existing.unshift(event);
  await store.setJSON('all', existing.slice(0, 500));
}

// ── Job Postings ─────────────────────────────────────────

function jobStore() {
  return getStore({ name: 'job-postings', consistency: 'strong' });
}

export async function getJobPostings(entityId?: string): Promise<JobPosting[]> {
  const store = jobStore();
  let all: JobPosting[] = ((await store.get('all', { type: 'json' })) as JobPosting[]) || [];
  if (entityId) all = all.filter(j => j.entity_id === entityId);
  return all.sort((a, b) => new Date(b.posted_date).getTime() - new Date(a.posted_date).getTime());
}

export async function addJobPostings(postings: JobPosting[]): Promise<number> {
  const store = jobStore();
  const existing: JobPosting[] = ((await store.get('all', { type: 'json' })) as JobPosting[]) || [];
  const existingUrls = new Set(existing.map(j => j.url));
  const unique = postings.filter(j => !existingUrls.has(j.url));
  if (unique.length === 0) return 0;
  const merged = [...unique, ...existing]
...
export async function getJobTrends(): Promise<JobTrend[]> {
  const store = jobStore();
  return ((await store.get('trends', { type: 'json' })) as JobTrend[]) || [];
}
